...
\item \textit{The likelihood equation (4.1) needs more details. Can we
  ...
  derived in relation ti the dispersion parameter using any
  parametrization. We obtained the Hessian matrix by central finite
  differences using the Richardson method implemented in
  \texttt{R}. Although the expression of reparametrized log-likelihood
  (Equation 3.1) seems more complicated, this function is better behaved
  than the function expressed in the original form (see Figure
  8). Consider the exponential family of distributions, with probability
  function given by
  ...
  canonical link function has no important rule here. In this case, the
  ...
  maps the parameter space and provide simple interpretations (in terms
  of rate ratios).
\item \textit{For model fitting, the authors used general purpose
  ...
  sizes. We reported these situations in the updated version of the paper.
  \begin{table}[H]
    \centering
    \begin{tabular}{lcccc}
      \hline
...
\item \textit{Numerical derivatives with CMP tend to be very slow and
  ...
  to choose of start values and normalizing constant upper bound. We
  set, for all models, the upper bound 500 and we start the algorithm
...
\item \textit{In Section 5, the simulated counts are ranging from 3 to
  ...
  the counts for $\mu=27$ for the different scenarios.
...
\item \textit{From the presentation point of view, focusing more on
  ...
  count data. These are the methodological novelties of the paper
...
compute the variances, that is showed in the Figure 3(a) and (b) we used
$\text{Var}(Y)=\sum_{y=0}^{500} y^2p(y) - [\sum_{y=0}^{500} y
p(y)]^2$.
...
current version of the paper.  \vspace{0.4cm}
...
analyze data generated according to the COM-Poisson distribution, using
the assumption $\text{Var}(Y_i)\propto \mu_i$. The advantage of the
COM-Poisson approach is that it corresponds to a fully specified
probability model allowing to compute the likelihood and some useful
measures like deviance, AIC, BIC, LRT, etc.
\vspace{0.4cm}
...
represent the expectation, nor approximately. The quadratic errors in
Figure 1(a) is obtained by $[a\text{E(Y)} - \sum_{y=0}^{500}y p(y)]^2$.
The models fitted in the case studies are
...
\item \textit{Consider the constraint ``$\mu > 0$'', after equation
  ...
  it does not prejudice the application of the model. We thank the
  referee for highlighting this point. We have discussed it in the
  updated version.
...
\item \textit{The Shmueli approximations to the mean and variance hold
  ...
  well. For examples presented in the paper, the parameter estimates
